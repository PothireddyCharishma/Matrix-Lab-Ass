...
class PairAddress extends Component {
  ...
  getApiDetails = async () => {
    ...
    const response = await fetch(
      `https://api.dexscreener.com/latest/dex/pairs/${searchInput}`,
    )
    const data = await response.json()
    this.setState({results: data.pairs})
  }
  ...
  render() {
    ...
    return (
      <div className="token-main-container">
        ...
        <h1 className="token-search-results-head">Pair Search Results </h1>
        <ul className="unorder-token-list">
          {results.map(each => (
            <DisplayPairs key={each.baseToken.address} details={each} />
          ))}
        </ul>
      </div>
    )
